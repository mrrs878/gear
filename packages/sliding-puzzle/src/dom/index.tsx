 ...
 * @LastEditTime: 2021-08-20 21:43:06
...
const Spin = (props: ISpinProps) => (
  props.spinning ? <div style={props.style}>loading...</div> : <></>
);
...
const MVerify = (props: IMVerifyProps) => {
  ...
  const [checkPuzzleLoading, setCheckPuzzleLoading] = useState(false);
  ...
  const handleDragMove = useCallback((e: any) => {
    ...
    if (newMoveX < 0 || newMoveX + 38 >= (props.containerSize?.width || 0)) return false;
    setMoveX(newMoveX);
    return true;
  }, [dragStatus, originPosition.x, props.containerSize]);
  ...
  const handleDragEnd = useCallback(async () => {
    try {
      ...
      setCheckPuzzleLoading(true);
      const res = await props.onRelease(moveX);
      setCheckPuzzleLoading(false);
      if (res) onSuccess();
      else onFail();
    } catch (e) {
      setCheckPuzzleLoading(false);
  ...
  return (
    <div className="puzzle-container">
      ...
      >
        ...
        />
        <img src={props.background} alt="" srcSet="" width={props.containerSize?.width} height={props.containerSize?.height} crossOrigin="anonymous" />
        ...
        <img src={props.block} alt="" srcSet="" className="puzzle-block" style={{ left: moveX || 10 }} />
        <span
        ...
        <div
          ...
          `}
        >
          <div style={{ width: `${moveX}px` }} className="puzzle-slider-mask" />
          <div style={{ left: `${moveX}px` }} onMouseDown={handleDragStart} className="puzzle-slider">
            {
